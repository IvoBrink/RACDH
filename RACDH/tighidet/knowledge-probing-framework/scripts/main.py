...
from src.classification.classifier import perform_classification_by_relation_group, save_classification_metrics, print_classification_summary
from src.classification.weighted_agg_classifier import perform_weighted_agg_logreg_classification, save_weighted_agg_metrics, print_weighted_agg_summary
...
if __name__ == "__main__":
    ...
    ##############################################################################################
    #### 7. WeightedAggLogReg probe on generation-time hidden states (attriwiki_probe) ##########
    ##############################################################################################
    weighted_agg_metrics = perform_weighted_agg_logreg_classification(
        prompting_results=prompting_results,
        model_name=model_name,
        device=device,
    )

    save_weighted_agg_metrics(
        metrics=weighted_agg_metrics,
        model_name=model_name,
        permanent_path=PERMANENT_PATH,
        test_on_head=test_on_head,
        n_head=n_head,
    )
    print_weighted_agg_summary(weighted_agg_metrics, model_name=model_name)
