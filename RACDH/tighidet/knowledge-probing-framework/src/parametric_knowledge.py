...
def build_parametric_knowledge(
    ...
    # Only materialise the first-token full distribution; it is the only one used downstream
    # (counter-object selection in context_knowledge.py indexes log_probas[0] by token ID).
    # Computing all max_gen_tokens distributions and storing them as Python lists blows up RAM
    # for large-vocab models (e.g. Qwen2.5-7B: 152k tokens × 10 tokens × N rows).
    first_token_log_probas = torch.log(torch.softmax(output_ids.scores[0], dim=-1))[0].tolist()

    # compute the log-likelihood on-the-fly without retaining the full distributions
    log_likelihood = 0.0
    nb_gen_tokens = len(output_ids.scores)
    for logits in output_ids.scores:
        log_probs_i = torch.log(torch.softmax(logits, dim=-1))[0]
        log_likelihood += float(log_probs_i[int(torch.argmax(log_probs_i))])
    ...
    return {
        ...
        "log_probas": (first_token_log_probas,),
