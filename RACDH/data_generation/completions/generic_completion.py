...
def add_generic_completion(entity, passage):
    ...
    if entity.lower() not in completion.lower():
        print_warning("Entity not found in model output. Returning None.")
        return None
...
def get_prompt(entity, passage):
    ...
    output_ex = "The author of the novel Frankenstein is named Mary Shelley."

    prompt = f"""Instruction: You are given a Wikipedia passage that contains an entity. Then you are given an entity's name. Produce one additional sentence that naturally appends to the passage, reaffirming the entity’s identity. You may use one of the following formats (or a similarly natural variant):
